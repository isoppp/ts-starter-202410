...
function autoGenerateExampleScalarsOrEnums({ seq }) {
    return {
        name: getScalarFieldValueGenerator().String({ modelName: "Example", fieldName: "name", isId: false, isUnique: false, seq })
    };
}
function defineExampleFactoryInternal({ defaultData: defaultDataResolver, onAfterBuild, onBeforeCreate, onAfterCreate, traits: traitsDefs = {} }, defaultTransientFieldValues) {
    const getFactoryWithTraits = (traitKeys = []) => {
        const seqKey = {};
        const getSeq = () => getSequenceCounter(seqKey);
        const screen = createScreener("Example", modelFieldDefinitions);
        ...
        const build = async (inputData = {}) => {
            const seq = getSeq();
            const requiredScalarData = autoGenerateExampleScalarsOrEnums({ seq });
        ...
        const create = async (inputData = {}) => {
            ...
            const createdData = await getClient().example.create({ data });
 ...
 */
export const defineExampleFactory = ((options) => {
    return defineExampleFactoryInternal(options ?? {}, {});
});
defineExampleFactory.withTransientFields = defaultTransientFieldValues => options => defineExampleFactoryInternal(options ?? {}, defaultTransientFieldValues);
